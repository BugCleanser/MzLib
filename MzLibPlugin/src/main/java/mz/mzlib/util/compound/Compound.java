            ...
            synchronized (wrapperClass)
            {
                ...
                for(Method method:wrapperClass.getMethods())
                {
                    ...
                    if(compoundOverride!=null)
                    {
                        ...
                        tar=(Method) WrapperClassInfo.get(RuntimeUtil.cast(tar.getDeclaringClass())).wrappedMembers.get(tar);
                        MethodNode mn=new MethodNode(Opcodes.ACC_PUBLIC, tar.getName(), AsmUtil.getDesc(method), null, null);
