...
public abstract class Coroutine
{
	...
	public static Coroutine init(AsyncFunction<?> function)
	{
		...
		for(int i=0;i<template.instructions.size();i++)
		{
			...
			if(insn instanceof IincInsnNode&&insn.getOpcode()==Opcodes.IINC||insn instanceof VarInsnNode&&(AsmUtil.opcodesVarLoad.contains(insn.getOpcode())||AsmUtil.opcodesVarStore.contains(insn.getOpcode()))) // $x
			{
				String type;
