...
public class ItemInfoSubcommand extends AbsLastCommandProcessor
{
	...
	public void execute(Player sender)
	{
		...
		if(!ItemStackBuilder.isAir(is))
		{
			...
			msg.add(TextMessageComponent.textCopy(StringUtil.replaceStrings(LangUtil.getTranslated(sender,"mzlib.command.iteminfo.name"),ListUtil.toMap(Lists.newArrayList(new MapEntry<>("%\\{locale\\}",sender.getLocale()),new MapEntry<>("%\\{name\\}",LangUtil.getTranslated(sender,ItemStackBuilder.getTranslateKey(is)))))),sender,ItemStackBuilder.getDropName(is,sender)));
