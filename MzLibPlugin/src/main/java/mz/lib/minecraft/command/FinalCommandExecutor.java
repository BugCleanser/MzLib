...
public class FinalCommandExecutor
{
	...
	{	
		...
		{
			sender.sendMessage(StringUtil.replaceStrings(MinecraftLanguages.get(sender,senderTypeError),ListUtil.toMap(Lists.newArrayList(new MapEntry<>("%\\{type\\}",this.senderType.getSimpleName())))));
			return;
		}
		args=StringUtil.mergeStrings(this.argParsers.size()-1,this.argParsers.size(),args);
	...
	{	
		...
		{	
			...
			if(this.argInfos[i].name().equals(""))
				...
				sb.append(MinecraftLanguages.get(player,this.argInfos[i].name()));
			sb.append('>');
		}
		return sb.toString();
	}
