            ...
            synchronized(wrapperClass)
            {
                ...
                for(Method method: wrapperClass.getMethods())
                {
                    ...
                    if(compoundSuper!=null)
                    {
                        ...
                        try
                        {
                            wrapper = compoundSuper.parent().getMethod(compoundSuper.method(), method.getParameterTypes());
                        }
                        ...
                        if(ElementSwitcher.isEnabled(wrapper))
                        {
                            ...
                            mn.visitMethodInsn(Opcodes.INVOKESPECIAL, AsmUtil.getType(tar.getDeclaringClass()), tar.getName(), AsmUtil.getDesc(tar), Modifier.isInterface(tar.getDeclaringClass().getModifiers()));
                    ...
                    if(compoundOverride!=null)
                    {
                        ...
                        try
                        {
                            wrapper = compoundOverride.parent().getMethod(compoundOverride.method(), method.getParameterTypes());
                        }
                        catch(NoSuchMethodException e)
                        {
                            throw new IllegalStateException("Wrapper method not found: "+method);
