...
public class SimpleTester<C extends TesterContext> implements Tester<C>
{
    ...
    public static class Builder<C extends TesterContext>
    {
        ...
        {
            data = new SimpleTester<>(
                "<unnamed>", contextType, 0, context -> CompletableFuture.completedFuture(Collections.emptyList()));
        }
        ...
        public SimpleTester<C> build()
        {
            return new SimpleTester<>(
                this.get().getName(), this.get().getContextType(), this.get().getMinLevel(), this.get().getFunction());
        }
    }
}
