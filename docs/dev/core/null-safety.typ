...
Java的*类型系统*并不原生区分`null`，在Java中实现空安全一般有两种方式：

+ 类似Rust包一层：`Optional`

+ 类似Kotlin的`T?`：使用*空安全注解*如`@Nullable T`
...
- Java标准库早已大量使用`null`且将来不会更改，例如`Map#get`
  而`Optional`无法良好兼容空安全注解：例如将*可空*泛型`T extends @Nullable Object`置于其中的`Optional<T>`是不正确的

- 由于JVM的方法*重载*基于泛型*擦除*，这会导致你的`Optional<T1>`和`Optional<T2>`在*描述符*上完全相同，容易导致冲突。
  这也就是为什么当你把`Optional`作为*参数*类型时会得到Idea的*警告*
...
注意你的泛型声明也变成默认非空的了：`<T>`相当于`<T extends @NotNull Object>`，若要允许可空泛型则需明确写成`<T extends @Nullable Object>`
...
#cardAttention[
    ...
    建议仅在*性能敏感*处使用*原始类型*的数组
]

同样的，在类型后加`[]`表示其数组类型，如`@Nullable Object[]`表示*元素可空*的数组

数组自身的注解标在`[]`之前，如`Object @Nullable[]`表示*自身可空*的数组

同理，`@Nullable Object @Nullable[]`表示自身和元素*皆可空*的数组

= `Box`

对于可空泛型`<T extends @Nullable Object>`，表示其*可选值*则不能直接使用`@Nullable T`，否则无法区分*不存在*和*存在空值*
...
为明确语义，我们提供了`Option`来代替`@Nullable Box`（注意不能换成`Optional`）：
```java
// 假设T为@Nullable Object
...
```
总之*混用*`@Nullable`和`Optional`是非常不推荐的，建议全面使用空安全注解
