    ...
    public static class Module extends MzModule
    {
        ...
        public void onLoad()
        {
            ...
            this.register(new EventListener<>(
                ...
                event -> instances.remove(event.getPlayer().toPlayer())
            ));
                ...
                synchronized(tracker)
                {
                    for(int entityId : packetEvent.getPacket().getEntityIds())
                    {
                        ...
                        if(displayEntity == null)
                            continue;
                        EventAsyncDisplayEntityDestroy event = new EventAsyncDisplayEntityDestroy(
                            displayEntity, packetEvent);
                        event.call();
                        if(!event.isCancelled())
                ...
                PacketS2cEntityData.FACTORY, Priority.LOW, packetEvent ->
            {
                ...
                synchronized(displayEntityTracker)
                {
                    DisplayEntity displayEntity = displayEntityTracker.entities.get(
                        packetEvent.getPacket().getEntityId());
                    if(displayEntity == null)
                        return;
