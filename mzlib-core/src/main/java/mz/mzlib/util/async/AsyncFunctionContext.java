...
public abstract class AsyncFunctionContext
{
    ...
    public static AsyncFunctionContext init(AsyncFunction<?> function)
    {
        try
        {
            ...
            cnContext.visit(
                cn.version, Opcodes.ACC_PUBLIC, AsmUtil.getType(function.getClass()) + "$0AsyncFunctionContext", null,
            ...
            mn.visitMethodInsn(
                Opcodes.INVOKESPECIAL, cnContext.superName, "<init>", AsmUtil.getDesc(void.class, new Class[0]), false);
            ...
            for(MethodNode i : cn.methods)
            {
                ...
                il.add(new FieldInsnNode(
                    Opcodes.PUTFIELD, AsmUtil.getType(AsyncFunction.class), "context",
                ...
                for(AbstractInsnNode insn : i.instructions)
                {
                    if(insn instanceof MethodInsnNode && insn.getOpcode() == Opcodes.INVOKESPECIAL &&
                        ...
                        ((MethodInsnNode) insn).owner.equals(AsmUtil.getType(AsyncFunction.class)))
                    {
            ...
            mn.visitFieldInsn(
                Opcodes.GETFIELD, AsmUtil.getType(AsyncFunction.class), "context",
                AsmUtil.getDesc(AsyncFunctionContext.class)
            );
            mn.visitFieldInsn(
            ...
            for(int i = 0; i < template.instructions.size(); i++)
            {
                ...
                if(insn instanceof IincInsnNode && insn.getOpcode() == Opcodes.IINC || insn instanceof VarInsnNode &&
                ...
                else if(insn instanceof MethodInsnNode)
                {
                            ...
                            Objects.equals(in.owner, AsmUtil.getType(function.getClass()))))
                    {
                        ...
                        if(Objects.equals(
                            in.desc, AsmUtil.getDesc(void.class, CompletableFuture.class))) // await(completableFuture);
                        {
                            mn.instructions.add(AsmUtil.insnVarLoad(AsyncFunction.class, 0));
                        ...
                        else // await(basicAwait);
                        {
                            mn.instructions.add(AsmUtil.insnVarLoad(AsyncFunction.class, 0)); // this
                            mn.visitFieldInsn(
                                Opcodes.GETFIELD, AsmUtil.getType(AsyncFunction.class), "runner",
            ...
            mn.instructions.add(AsmUtil.insnPop(boolean.class));
            mn.instructions.add(AsmUtil.insnReturn(void.class));
            @SuppressWarnings("deprecation") CastChecker checker = new CastChecker(mn);
            checker.analyze();
            checker.autoCast();
