...
public class LangUtil extends AbsModule implements IRegistrar<LangUtil.LangFolder>
{
	...
	public JsonObject getVersionInfo()
	{
		if(versionInfo!=null)
			return versionInfo;
		File f=new File(new File(MzLib.instance.getDataFolder(),"MCVersionInfo"),BukkitWrapper.MCVersion+".json");
		if(f.exists())
		{
			try(FileInputStream fis=new FileInputStream(f))
			{
		...
		try
		{
			...
			{
				if(v.getAsJsonObject().get("id").getAsString().equals(BukkitWrapper.MCVersion))
				{
	...
	public JsonObject getVersionAssets()
	{
		...
		File f=new File(new File(MzLib.instance.getDataFolder(),"MCVersionAssets"),BukkitWrapper.MCVersion+".json");
		if(f.exists())
		{
			try(FileInputStream fis=new FileInputStream(f))
			{
	...
	{
		File f=new File(new File(new File(MzLib.instance.getDataFolder(),"MCAssets"),BukkitWrapper.MCVersion),file);
		if(f.exists())
		{
			try(FileInputStream fis=new FileInputStream(f))
			{
