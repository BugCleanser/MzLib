...
public interface WrittenBookContentComponentV2005 extends WrapperObject
{
    ...
    {
        return this.getPages0().stream().map(RawFilteredPairV2005.FACTORY::create).map(p->p.get(false, Text.FACTORY)).collect(Collectors.toList());
    }
    
    @WrapMinecraftFieldAccessor(@VersionName(name="comp_2423"))
    boolean isResolved();
    
    static WrittenBookContentComponentV2005 newInstance(String title, String author, int generation, List<Text> pages, boolean resolved)
    {
        return newInstance0(RawFilteredPairV2005.newInstance0(title, Optional.empty()), author, generation, pages.stream().map(p->RawFilteredPairV2005.newInstance(p, Option.none()).getWrapped()).collect(Collectors.toList()), resolved);
    }
    ...
    {
        return newInstance0(this.getTitle0(), this.getAuthor(), this.getGeneration(), pages.stream().map(p->RawFilteredPairV2005.newInstance(p, Option.none()).getWrapped()).collect(Collectors.toList()), this.isResolved());
    }
