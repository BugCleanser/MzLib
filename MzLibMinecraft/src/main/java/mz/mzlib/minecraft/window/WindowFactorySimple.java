...
public interface WindowFactorySimple extends WindowFactory
{
    ...
    static WindowFactorySimple newInstance(
        String windowIdV_1400,
        Text displayName,
        BiFunction<Integer, InventoryPlayer, Window> windowCreator)
    {
    ...
    static WindowFactorySimple newInstance(
        ...
        Text displayName,
        BiFunction<Integer, InventoryPlayer, Window> initializer)
    {
        return newInstance(windowType.typeIdV_1400, displayName, initializer);
    }

    static WindowFactorySimple chest(
        Text title,
        Inventory inventory,
        int rows,
        Consumer<? super WindowChest> initializer)
    {
        return newInstance(
            WindowType.generic9x(rows), title, (syncId, inventoryPlayer) ->
            {
