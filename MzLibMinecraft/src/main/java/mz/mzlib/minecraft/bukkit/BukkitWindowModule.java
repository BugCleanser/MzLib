...
public class BukkitWindowModule extends MzModule
{
    ...
    {
        @WrapMethod("@0")
        boolean placeInOrCheck(ItemStack itemStack, int begin, int end, boolean inverted, boolean doCheck);
        
        @NothingInject(wrapperMethod="placeInOrCheck", type=NothingInjectType.INSERT_BEFORE, locateMethod="")
        default WrapperBoolean placeInOrCheckOverwrite(@LocalVar(1) ItemStack itemStack, @LocalVar(2) int begin, @LocalVar(3) int end, @LocalVar(4) boolean inverted, @LocalVar(5) boolean doCheck)
        {
