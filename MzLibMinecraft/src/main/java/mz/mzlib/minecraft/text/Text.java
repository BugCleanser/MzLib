...
public interface Text extends WrapperObject
{
    ...
    default String toLegacy(TextStyle parentStyle)
    {
        ...
        switch(this.getType())
        {
            ...
            case TRANSLATABLE:
                ...
                literal = textTranslatable.getKey() + '[' + textTranslatable.getArgs().stream()
                    .map(a -> a instanceof Text ? ((Text) a).toLegacy(withParent) : a.toString())
    ...
    class Module extends MzModule
    {
        ...
        public void onLoad()
        {
            SimpleTester.Builder<TesterContext> testBuilder = SimpleTester.Builder.common()
                .setName(Text.class.getName()).setMinLevel(1);
            ...
            this.register(testBuilder.setFunction(context ->
            {
                ...
                TextTranslatable text = codec.apply(MinecraftPlatform.instance.getVersion() < 1904 ?
                    TextTranslatable.newInstance(key, arg0, arg1) :
                    TextTranslatable.newInstanceV1904(key, fallbackV1904, arg0, arg1)).as(TextTranslatable.FACTORY);
                ...
                if(!text.getArgs().get(1).equals(arg1.getLiteral()) &&
                    !((Text) text.getArgs().get(1)).as(TextLiteral.FACTORY).getLiteral().equals(arg1.getLiteral()))
            ...
            this.register(testBuilder.setFunction(context ->
            {
                ...
                TextSelector text = codec.apply(MinecraftPlatform.instance.getVersion() < 1700 ?
                    TextSelector.newInstance(selector) :
                    TextSelector.newInstanceV1700(selector, Option.some(separator))).as(TextSelector.FACTORY);
                ...
                if(MinecraftPlatform.instance.getVersion() >= 1700)
                {
                    if(!text.getSeparatorV1700().unwrap().as(TextLiteral.FACTORY).getLiteral()
                        .equals(separator.getLiteral()))
            ...
            if(MinecraftPlatform.instance.getVersion() >= 2109)
                this.register(testBuilder.setFunction(context ->
                {
                    TextObjectV2109 text = codec.apply(TextObjectV2109.atlas(
                        Identifier.minecraft(randomString.get()),
                        Identifier.minecraft(randomString.get())
                    )).as(TextObjectV2109.FACTORY);
                    ...
                    text = codec.apply(TextObjectV2109.player(
                        GameProfile.Description.texturesUrl(
    ...
    @VersionRange(end = 2106)
    @WrapMinecraftInnerClass(outer = Text.class, name = {
        @VersionName(name = "Serializer", end = 2003),
        @VersionName(name = "Serialization", begin = 2003)
    })
    interface SerializerV_2106 extends WrapperObject
    {
        ...
        @WrapMinecraftMethod({
            ...
            @VersionName(name = "fromJson", begin = 1400)
        })
