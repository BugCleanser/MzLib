...
public class LangEditor extends UIWrittenBook
{
    ...
    public LangEditor(String lang, String node)
    {
        ...
        if(this.node!=null)
        {
            ...
            this.buttonSet=this.newButton(player->UIWindowAnvilInput.invoke(player, I18n.getTranslation(this.lang, this.node, "").replace("&", "&&").replace("§", "&"), Text.literal(String.format(I18nMinecraft.getTranslation(player, "mzlib.lang.editor.homepage.custom.set.title"), this.node))).whenComplete((r,e)->
            {
                if(e!=null)
                    throw RuntimeUtil.sneakilyThrow(e);
                if(r!=null)
                {
                    I18n.custom.map.computeIfAbsent(this.lang, k->new ConcurrentHashMap<>()).put(this.node, r.replace("&", "§").replace("§§", "&"));
