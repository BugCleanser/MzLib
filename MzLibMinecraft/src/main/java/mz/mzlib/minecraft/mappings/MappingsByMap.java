...
public class MappingsByMap extends Mappings<MappingsByMap>
{
    ...
    {
        return Optional.ofNullable(this.fields.get(fromClass)).map(it -> it.getOrDefault(fromField, fromField))
            .orElse(null);
    }
    ...
    {
        return Optional.ofNullable(this.methods.get(fromClass)).map(it -> it.getOrDefault(fromMethod, fromMethod.name))
            .orElse(null);
    }

    // TODO: parse intermediary mappings
    public static MappingsByMap parseYarnLegacy(String mappings)
    {
        MappingsByMap result = new MappingsByMap();
        Iterator<String> it = new BufferedReader(new StringReader(mappings)).lines().filter(_it -> !_it.startsWith("#"))
            .iterator();
        ...
        for(String line = it.next(); line != null; line = it.hasNext() ? it.next() : null)
        {
            ...
            switch(a[0])
            {
                ...
                case "FIELD":
                    ...
                    break;
                case "METHOD":
                    result.methods.computeIfAbsent(a[1].replace('/', '.'), it1 -> new HashMap<>())
                        .put(MappingMethod.parse(a[3], a[2]), a[5]);
                    break;
                default:
    ...
    public static MappingsByMap parseZipMapping(ZipInputStream mappings, String prefix)
    {
        MappingsByMap result = new MappingsByMap();
        try
        {
            for(ZipEntry i = mappings.getNextEntry(); i != null; i = mappings.getNextEntry())
            {
                ...
                {
                    Iterator<String> it = new BufferedReader(new StringReader(
                        new String(MappingsUtil.readInputStream(mappings), StandardCharsets.UTF_8))).lines()
                        .filter(_it -> !_it.trim().startsWith("COMMENT")).iterator();
                    ...
                    while(it.hasNext())
                    {
                        ...
                        switch(a[0])
                        {
                            ...
                            case "METHOD":
                                if(a.length >= 4)
                                    result.methods.computeIfAbsent(from.get(), it1 -> new HashMap<>())
                                        .put(MappingMethod.parse(a[1], a[3]), a[2]);
                                break;
                            case "ARG":
                                break;
                            default:
    ...
    public static MappingsByMap parseTiny(String mappings)
    {
        MappingsByMap result = new MappingsByMap();
        Iterator<String> it = new BufferedReader(new StringReader(mappings)).lines().filter(_it -> !_it.startsWith("#"))
            .iterator();
        ...
        while(it.hasNext())
        {
            ...
            switch(a[0])
            {
                ...
                case "METHOD":
                    result.methods.computeIfAbsent(a[1].replace('/', '.'), it1 -> new HashMap<>())
                        .put(MappingMethod.parse(a[3], a[2]), a[4]);
                    break;
                default:
                    throw new UnsupportedOperationException(line);
            }
    ...
    public static MappingsByMap parseMojang(String raw)
    {
        MappingsByMap result = new MappingsByMap();
        Iterator<String> lines = new BufferedReader(new StringReader(raw)).lines()
            .filter(l -> !l.isEmpty() && !l.startsWith("#")).iterator();
    ...
    public static MappingsByMap parseCsrg(String classMappings, String memberMappings)
    {
        MappingsByMap result = new MappingsByMap();
        new BufferedReader(new StringReader(classMappings)).lines().filter(l -> !l.startsWith("#")).forEach(line ->
        {
        ...
        new BufferedReader(new StringReader(memberMappings)).lines().filter(l -> !l.startsWith("#"))
            .forEach(it1 -> handleMember(result, it1));

        return result;
    }
    public static void handleMember(MappingsByMap mappings, String str)
    {
        ...
        else if(line.length == 4)
        {
            ...
            mappings.methods.computeIfAbsent(clazz, it -> new HashMap<>())
                .put(MappingMethod.parse(deobfName, signature), obfName);
        }
    }
        ...
        for(Map.Entry<String, Map<MappingMethod, String>> e : this.methods.entrySet())
        {
            ...
            for(Map.Entry<MappingMethod, String> e1 : e.getValue().entrySet())
            {
                rm.put(
                    new MappingMethod(
                        e1.getValue(),
                        Arrays.stream(e1.getKey().parameterTypes).map(this::processDesc).toArray(String[]::new)
                    ), e1.getKey().name
                );
            }
        }
