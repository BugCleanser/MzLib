...
public class MappingsByMap extends Mappings<MappingsByMap>
{
        ...
        while(line!=null)
        {
            ...
            while(true)
            {
                ...
                if(from.endsWith(")"))
                {
                    ...
                    result.methods.computeIfAbsent(cl[0], it->new HashMap<>()).put(new MappingMethod(c[0], Arrays.stream(c[1].substring(0, c[1].length()-1).split(",")).map(MappingsByMap::type2desc).toArray(String[]::new)), to);
                }
                else
                {
    ...
    public static MappingsByMap parseCsrg(String classMappings, String memberMappings)
    {
        ...
        new BufferedReader(new StringReader(memberMappings)).lines().filter(l->!l.startsWith("#")).forEach(it1->handleMember(result, it1));
