    ...
    default Option<List<Text>> static$getLoreV_1400(ItemStack itemStack)
    {
        for(NbtCompound tag : itemStack.getTagV_2005())
        {
            for(NbtCompound display : tag.getNbtCompound("display"))
            {
                ...
                {
                    return Option.some(
                        lore.asList(NbtString.FACTORY).stream().map(NbtString::getValue).map(Text::fromLegacy)
    ...
    default Option<List<Text>> static$getLoreV1400_2005(ItemStack itemStack)
    {
        for(NbtCompound tag : itemStack.getTagV_2005())
        {
            for(NbtCompound display : tag.getNbtCompound("display"))
            {
                ...
                {
                    return Option.some(
                        lore.asList(NbtString.FACTORY).stream().map(NbtString::getValue).map(Text::decode)
                            .collect(Collectors.toList()));
                }
            }
        }
                ...
                if(value.isSome())
                    display.put(
                        "Lore", NbtList.newInstance(
                            value.unwrap().stream().map(Text::toLegacy).map(NbtString::newInstance)
        ...
        for(NbtCompound tag : reviseCustomData(itemStack))
        {
            for(NbtCompound display : tag.reviseNbtCompoundOrNew("display"))
            {
                if(value.isSome())
                    display.put(
                        "Lore", NbtList.newInstance(
                            value.unwrap().stream().map(Text::encode).map(JsonElement::toString)
